...
public class AStar15Puzzle {
    ...
    public static PartialSolution solveByAStar(Board board) {
        // TODO 1.3 solveAStar
        PriorityQueue<PartialSolution> sollutions = new PriorityQueue<PartialSolution>();
        sollutions.add(new PartialSolution(board));
        while (!sollutions.isEmpty()){
            PartialSolution psol = sollutions.poll();
            if (psol.isSolution()){
                return psol;
            }
            for (Move move : psol.validMoves()){
                PartialSolution clone = new PartialSolution(psol);
                clone.doMove(move);
                sollutions.add(clone);
            }
        }
        return null;
    }
