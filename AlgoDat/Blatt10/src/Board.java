...
public class Board {
    ...
    public int manhattan() {
        ...
        for (Position pos : positions) {
            if (getField(pos) != BLANK) {
                int num = getField(pos);
                int supposedX = (num % (nx)) - 1;
                int supposedY = num/nx;
                if (num % nx == 0) {
                    supposedX = nx - 1;
                    supposedY = supposedY - 1;
                }
                int man = (int) (Math.abs(pos.x - supposedX) + Math.abs(pos.y - supposedY));
                if (supposedX != pos.x || supposedY != pos.y) {
                    result += man;
                }
            }
        }
