...
def power_iteration(M: np.ndarray, epsilon: float = -1.0) -> (np.ndarray, list):
    ...
    if epsilon < 0:
        epsilon = np.finfo(float).eps * 5.0
    # TODO: normalized random vector of proper size to initialize iteration
    vector = np.random.randn(M.shape[0]) / np.linalg.norm(np.random.randn(M.shape[0]))
    ...
    while residual > epsilon:
        ...
        residual = np.linalg.norm(mat - vector)
        vector = mat
...
def load_images(path: str, file_ending: str = ".png") -> (list, int, int):
    ...
    for img in sorted(lib.list_directory(path)):
        if img.endswith(file_ending):
            images.append(np.asarray(matplotlib.image.imread(path + img), np.float64))
...
def accumulated_energy(singular_values: np.ndarray, threshold: float = 0.8) -> int:
    ...
    sum_th = threshold * np.sum(singular_values)
    i = 0
    while i < sum_th:
        i = i + singular_values[k]
        k = k + 1
...
def identify_faces(coeffs_train: np.ndarray, pcs: np.ndarray, mean_data: np.ndarray, path_test: str) -> (
    ...
    for i in range(scores.shape[0]):
        for j in range(scores.shape[1]):
            scores[i, j] = np.arccos(np.clip(np.dot(coeffs_train[i] / np.linalg.norm(coeffs_train[i]),coeffs_test[j] / np.linalg.norm(coeffs_test[j]) ), -1.0, 1.0))
